...
def test_single_case(net, image1, image2, image3, stride_x, stride_y, stride_z, patch_size, num_classes=1):
    w, h, d = image1.shape
    ...
    if add_pad:
        ...
        image2 = np.pad(image2, [(wl_pad, wr_pad), (hl_pad, hr_pad), (dl_pad, dr_pad)], mode='constant',
                        constant_values=0)
        image3 = np.pad(image3, [(wl_pad, wr_pad), (hl_pad, hr_pad), (dl_pad, dr_pad)], mode='constant',
                        constant_values=0)
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                test_patch = np.expand_dims(np.concatenate(
                    (np.expand_dims(test_patch1, axis=0),
