...
from utils.sliding_window_inference_2channels import test_single_case
from utils.losses import dice_loss
from dataloaders.BC_2channels import BC
from dataloaders.BC_2channels import Crop2patchesToTensor
from dataloaders.data_augmentation_2channels import *
from networks.UNet3D import UNet3D_DS
from networks.UNet3D_se import UNet3D_DS_SE
from networks.nnResUnet import nnResUNet
from networks.nnResUnet_se import nnResUNet_SE
from networks.VNet import VNet_DS
from networks.VNet_se import VNet_DS_SE


# 参数设置
parser = argparse.ArgumentParser()
parser.add_argument("--exp_name", type=str, default='debug1', help='experiment name used for save')
...
parser.add_argument("--max_iterations", type=int, default=20000, help="number of iterations of training")
...
parser.add_argument("--img1_prefix", type=str, default='CROPDCE-C0-185', help="img prefix")
...
parser.add_argument("--note", type=str, default="1&Sub_fold0", help="note")
...
def train(exp_name,
    ...
    eval_dataset = BC(eval_list, data_root_path, img1_prefix, img2_prefix, label_prefix)
    ...
    # model = nnResUNet(in_channels=2, out_channels=2, is_dropput=False).cuda()
    model = nnResUNet_SE(in_channels=2, out_channels=2, is_dropput=False).cuda()

    # model = UNet3D_DS(2, 2).cuda()
    # model = UNet3D_DS_SE(2, 2).cuda()
    # model = VNet_DS(in_channels=2, out_channels=2, normalization='groupnorm').cuda()
    # model = VNet_DS_SE(in_channels=2, out_channels=2, normalization='groupnorm').cuda()
        ...
        for i_batch, sampled_batch in enumerate(train_dataloader):
            ...
            if n_total_iter % n_eval_iter == 0:
                ...
                for eval_sample in tqdm(eval_dataloader):
                    ...
                    prediction, score_map = test_single_case(model, eval_input1, eval_input2,
