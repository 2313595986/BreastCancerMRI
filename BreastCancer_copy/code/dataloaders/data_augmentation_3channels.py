...
class GaussianNoiseTransform(object):
    ...
    def __init__(self, noise_variance=(0, 0.1), volume_key1='volume1', volume_key2='volume2', volume_key3='volume3', p_per_sample=1.0):
    ...
    def __call__(self, sample):
        volume1 = sample[self.volume_key1]
        volume2 = sample[self.volume_key2]
        volume3 = sample[self.volume_key3]
        if np.random.uniform() < self.p_per_sample:
            volume1, volume2, volume3 = self.augment_gaussian_noise([volume1, volume2, volume3], self.noise_variance)
        sample[self.volume_key1] = volume1
        sample[self.volume_key2] = volume2
        sample[self.volume_key3] = volume3
        return sample
...
class GaussianBlurTransform(object):
    def __init__(self, blur_sigma=(1, 5), different_sigma_per_channel=True,
                 p_per_channel=1, volume_key1='volume1', volume_key2='volume2', volume_key3='volume3', p_per_sample=1.0):
        """
        :param blur_sigma:
    ...
    def __call__(self, sample):
        ...
        if np.random.uniform() < self.p_per_sample:
            volume1, volume2, volume3 = self.augment_gaussian_blur([volume1, volume2, volume3], self.blur_sigma,
...
class BrightnessMultiplicativeTransform(object):
    def __init__(self, multiplier_range=(0.5, 2), per_channel=True, volume_key1='volume1', volume_key2='volume2',
                 volume_key3='volume3', p_per_sample=1.0):
        """
    ...
    def __call__(self, sample):
        volume1, volume2, volume3 = sample[self.volume_key1], sample[self.volume_key2], sample[self.volume_key3]
        if np.random.uniform() < self.p_per_sample:
            volume1, volume2, volume3 = self.augment_brightness_multiplicative([volume1, volume2, volume3],
...
class GammaTransform(object):
    def __init__(self, gamma_range=(0.5, 2), invert_image=False, per_channel=False, retain_stats=False,
                 volume_key1='volume1', volume_key2='volume2', volume_key3='volume3', p_per_sample=1):
        """
    ...
    def __call__(self, sample):
        volume1, volume2, volume3 = sample[self.volume_key1], sample[self.volume_key2], sample[self.volume_key3]
        if np.random.uniform() < self.p_per_sample:
            volume1, volume2, volume3 = self.augment_gamma([volume1, volume2, volume3], self.gamma_range,
...
class ContrastAugmentationTransform(object):
    def __init__(self, contrast_range=(0.75, 1.25), preserve_range=True, per_channel=True,
                 volume_key1='volume1', volume_key2='volume2', volume_key3='volume3', p_per_sample=1.0):
        """
    ...
    def __call__(self, sample):
        volume1, volume2, volume3 = sample[self.volume_key1], sample[self.volume_key2], sample[self.volume_key3]
        if np.random.uniform() < self.p_per_sample:
            volume1, volume2, volume3 = self.augment_contrast([volume1, volume2, volume3],
...
class RandomRotateTransform(object):
    def __init__(self, angle_range=(-10, 10), volume_key1='volume1', volume_key2='volume2', volume_key3='volume3',
    ...
    def __call__(self, sample):
        ...
        if np.random.uniform() < self.p_per_sample:
            ...
            volume1 = rotate(volume1, angle=rand_angle, axes=(1, 0), reshape=False, order=1)
            volume2 = rotate(volume2, angle=rand_angle, axes=(1, 0), reshape=False, order=1)
            volume3 = rotate(volume3, angle=rand_angle, axes=(1, 0), reshape=False, order=1)
...
class ScaleTransform(object):
    def __init__(self, zoom_range=(0.8, 1.3), volume_key1='volume1', volume_key2='volume2', volume_key3='volume3',
                 label_key='label', p_per_sample=1.0):
        self.p_per_sample = p_per_sample
    ...
    def __call__(self, sample):
        ...
        if np.random.uniform() < self.p_per_sample:
            ...
            volume1 = zoom(volume1, zoom_factor, order=1)
            volume2 = zoom(volume2, zoom_factor, order=1)
            volume3 = zoom(volume3, zoom_factor, order=1)
...
class MirrorTransform(object):
    def __init__(self, axes=(0, 1, 2), volume_key1='volume1', volume_key2='volume2', volume_key3='volume3',
                 label_key='label'):
        self.volume_key1 = volume_key1
        self.volume_key2 = volume_key2
        self.volume_key3 = volume_key3
    ...
    def __call__(self, sample):
        ...
        if isinstance(self.axes, int):
            if np.random.uniform() < 0.5:
                ...
                volume3 = np.flip(volume3, self.axes)
            ...
            for axis in self.axes:
                if np.random.uniform() < 0.5:
                    ...
                    volume3 = np.flip(volume3, axis=axis)
