...
class GaussianNoiseTransform(object):
    ...
    def __init__(self, noise_variance=(0, 0.1), volume_key='volume1', sub_volume_key='volume2', p_per_sample=1.0):
    ...
    def __call__(self, sample):
        ...
        sub_volume = sample[self.sub_volume_key]
        if np.random.uniform() < self.p_per_sample:
            volume, sub_volume = self.augment_gaussian_noise([volume, sub_volume], self.noise_variance)
        sample[self.volume_key] = volume
        sample[self.sub_volume_key] = sub_volume
        return sample
...
class GaussianBlurTransform(object):
    def __init__(self, blur_sigma=(1, 5), different_sigma_per_channel=True,
                 p_per_channel=1, volume_key='volume1', sub_volume_key='volume2', p_per_sample=1.0):
        """
        :param blur_sigma:
    ...
    def __call__(self, sample):
        volume, sub_volume = sample[self.volume_key], sample[self.sub_volume_key]
        if np.random.uniform() < self.p_per_sample:
            volume, sub_volume = self.augment_gaussian_blur([volume, sub_volume], self.blur_sigma,  self.different_sigma_per_channel, self.p_per_channel)
        sample[self.volume_key], sample[self.sub_volume_key] = volume, sub_volume
        return sample
...
class BrightnessMultiplicativeTransform(object):
    def __init__(self, multiplier_range=(0.5, 2), per_channel=True, volume_key='volume1', sub_volume_key='volume2',
                 p_per_sample=1.0):
        """
    ...
    def __call__(self, sample):
        volume, sub_volume = sample[self.volume_key], sample[self.sub_volume_key]
        if np.random.uniform() < self.p_per_sample:
            volume, sub_volume = self.augment_brightness_multiplicative([volume, sub_volume], self.multiplier_range, self.per_channel)
        sample[self.volume_key], sample[self.sub_volume_key] = volume, sub_volume
        return sample
...
class GammaTransform(object):
    def __init__(self, gamma_range=(0.5, 2), invert_image=False, per_channel=False, retain_stats=False,
                 volume_key='volume1', sub_volume_key='volume2', p_per_sample=1):
        """
    ...
    def __call__(self, sample):
        volume, sub_volume = sample[self.volume_key], sample[self.sub_volume_key]
        if np.random.uniform() < self.p_per_sample:
            volume, sub_volume = self.augment_gamma([volume, sub_volume], self.gamma_range, self.invert_image,
...
class ContrastAugmentationTransform(object):
    def __init__(self, contrast_range=(0.75, 1.25), preserve_range=True, per_channel=True,
                 volume_key='volume1', sub_volume_key='volume2', p_per_sample=1.0):
        """
    ...
    def __call__(self, sample):
        ...
        sub_volume = sample[self.sub_volume_key]
        if np.random.uniform() < self.p_per_sample:
            volume, sub_volume = self.augment_contrast([volume, sub_volume], contrast_range=self.contrast_range,
...
class RandomRotateTransform(object):
    def __init__(self, angle_range=(-10, 10), volume_key='volume1', label_key='label', sub_volume_key='volume2',
                 p_per_sample=1.0):
        self.p_per_sample = p_per_sample
    ...
    def __call__(self, sample):
        volume, label, sub_volume = sample[self.volume_key], sample[self.label_key], sample[self.sub_volume_key]
        if np.random.uniform() < self.p_per_sample:
            ...
            sub_volume = rotate(sub_volume, angle=rand_angle, axes=(1, 0), reshape=False, order=1)
...
class ScaleTransform(object):
    def __init__(self, zoom_range=(0.8, 1.3), volume_key='volume1', sub_volume_key='volume2', label_key='label',
                 p_per_sample=1.0):
        self.p_per_sample = p_per_sample
    ...
    def __call__(self, sample):
        ...
        if np.random.uniform() < self.p_per_sample:
            ...
            sub_volume = zoom(sub_volume, zoom_factor, order=1)
            label = zoom(label, zoom_factor, order=0)
        sample[self.volume_key], sample[self.sub_volume_key], sample[self.label_key] = volume, sub_volume, label

        return sample


class MirrorTransform(object):
    def __init__(self, axes=(0, 1, 2), volume_key='volume1', label_key='label', sub_volume_key='volume2'):
        self.volume_key = volume_key
        self.label_key = label_key
        self.sub_volume_key = sub_volume_key
        self.axes = axes
    ...
    def __call__(self, sample):
        ...
        if isinstance(self.axes, int):
            if np.random.uniform() < 0.5:
                ...
                sub_volume = np.flip(sub_volume, self.axes)
        else:
            for axis in self.axes:
                if np.random.uniform() < 0.5:
                    volume = np.flip(volume, axis=axis)
                    label = np.flip(label, axis=axis)
                    sub_volume = np.flip(sub_volume, axis=axis)
        sample[self.volume_key], sample[self.label_key], sample[self.sub_volume_key] = volume, label, sub_volume
        return sample