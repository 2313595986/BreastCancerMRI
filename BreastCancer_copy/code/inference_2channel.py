...
from networks.nnResUnet_BreastSeg import nnResUNet as BreastSegModel
from networks.nnResUnet_ShareEncoder import nnResUNet as TumorSegModel
...
def crop_xyz_axis_for_step3(volume_np, padding_size=10):
    x = 0
    while (volume_np[x, :, :] == 0).all():
        x += 1
        if x == volume_np.shape[0]:
            raise ValueError('prediction map doesnt have object, please check!')
    start_x = x - padding_size
    if start_x < 0:
        start_x = 0
    ...
    while (volume_np[x, :, :] == 0).all():
        x -= 1
    end_x = x + padding_size + 1
    if end_x > volume_np.shape[0]:
        end_x = volume_np.shape[0]

    y = 0
    while (volume_np[:, y, :] == 0).all():
        y += 1
    start_y = y - padding_size
    if start_y < 0:
        start_y = 0
    ...
    while (volume_np[:, y, :] == 0).all():
        y -= 1
    end_y = y + padding_size + 1
    if end_y > volume_np.shape[1]:
        end_y = volume_np.shape[1]

    z = 0
    while (volume_np[:, :, z] == 0).all():
        z += 1
    start_z = z - padding_size
    if start_z < 0:
        start_z = 0
    ...
    while (volume_np[:, :, z] == 0).all():
        z -= 1
    end_z = z + padding_size + 1
    if end_z > volume_np.shape[2]:
        end_z = volume_np.shape[2]

    crop_volume_np = volume_np[start_x:end_x, start_y:end_y, start_z:end_z]
    return crop_volume_np, start_x, end_x, start_y, end_y, start_z, end_z
...
if __name__ == '__main__':
    ...
    test_root_path = '/raid/hra/dataset/BreastCancerMRI1000/YN_6/RawData'
    test_list = get_test_list('../data/test_6.txt')
    output_root_dir = '../data/YN6/'

    model1_path = '../model/end2end_2channel/BreastSegModel.pth'
    model2_path = '../model/end2end_2channel/TumorSegModel.pth'
    ...
    for test_id in test_list:
        ...
        c0_path = os.path.join(test_root_path, 'DCE-C0', 'DCE-C0_%s' % test_id)
        ...
        c1_volume_crop, c0_volume_crop, crop_start_index_step1 = crop_y_axis_for_step1(c1_volume_np, c0_volume_np)
        ...
        c0_volume_crop_down = interpolation.zoom(c0_volume_crop, (0.36, 0.36, 1), order=3)
        # norm
        c1_volume_crop_down_norm = norm(c1_volume_crop_down)
        c0_volume_crop_down_norm = norm(c0_volume_crop_down)
        # time
        end_time3 = time.time()
        ...
        breast_mask, _ = inference_window_slice_1channel(model1, c1_volume_crop_down_norm, 100, 100, 30, (256, 256, 64), num_classes=2)
        ...
        c0_breast_roi = breast_mask * c0_volume_crop
        # crop zero
        c1_breast_roi_crop, c0_breast_roi_crop, crop_start_index_step2, crop_end_index_step2 \
            = crop_y_axis_for_step2(c1_breast_roi, c0_breast_roi)
        # downsample c1 c0 volume (*0.5 for xy)
        c1_breast_roi_crop_down = interpolation.zoom(c1_breast_roi_crop, (0.5, 0.5, 1), order=3)
        c0_breast_roi_crop_down = interpolation.zoom(c0_breast_roi_crop, (0.5, 0.5, 1), order=3)
        # norm
        c1_breast_roi_crop_down_norm = norm(c1_breast_roi_crop_down)
        c0_breast_roi_crop_down_norm = norm(c0_breast_roi_crop_down)
        # generate tumor label
        tumor_mask, _ = inference_window_slice_2channel(model2, c1_breast_roi_crop_down_norm, c0_breast_roi_crop_down_norm,
