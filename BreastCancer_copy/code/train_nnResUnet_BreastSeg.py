...
parser.add_argument("--exp_name", type=str, default='BreastSeg_c1_fold1_resunet_1', help='experiment name used for save')
...
parser.add_argument("--img_prefix", type=str, default='DCE-C1-185', help="img prefix")
parser.add_argument("--label_prefix", type=str, default='BreastMask-185', help="label prefix")
...
parser.add_argument("--note", type=str, default="BreastSeg_ResUnetMini_CropZerodown0.5XYZ_fold0", help="note")
...
def train(exp_name,
    ...
    eval_dataset = BC(eval_list, data_root_path, img_prefix, label_prefix)
    ...
    model = nnResUNet(in_channels=1, out_channels=2, is_dropput=False).cuda()
    # model = UNet3D_DS(1, 2).cuda()
    # model = VNet_DS(in_channels=1, out_channels=2, normalization='groupnorm').cuda()
    # model = ResVNet_DS(in_channels=1, out_channels=2, normalization='groupnorm').cuda()
    ...
    for epoch in range(max_epoch):
        ...
        for i_batch, sampled_batch in enumerate(train_dataloader):
            ...
            for index, seg_output in enumerate(seg_output_list):
                ...
                # remove dice loss 2021/10/12 16:12
                # seg_loss = seg_loss_ce
            ...
            # loss = seg_loss_list[0]
            # loss_CE = ce_loss_list[0]
            # loss_DICE = dice_loss_list[0]
            ...
            if n_total_iter % n_eval_iter == 0:
                ...
                for eval_sample in tqdm(eval_dataloader):
                    ...
                    prediction, score_map = test_single_case(model, eval_input,
