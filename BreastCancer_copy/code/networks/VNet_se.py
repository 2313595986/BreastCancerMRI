...
class DownsamplingConvBlock(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, stride=2, normalization='none'):
        ...
        self.conv = nn.Sequential(*ops)
        self.SE_block = SEblock(n_filters_out, reduction=16)
...
class VNet_DS_SE(nn.Module):
    def __init__(self, in_channels=3, out_channels=2, n_filters=16, normalization='none', has_dropout=False):
        super(VNet_DS_SE, self).__init__()
        self.has_dropout = has_dropout
        ...
        self.dropout = nn.Dropout3d(p=0.5, inplace=False)
        # self.__init_weight()
        self.out1 = nn.Conv3d(n_filters * 8, out_channels, kernel_size=(1, 1, 1), stride=(1, 1, 1), bias=False)
        self.out2 = nn.Conv3d(n_filters * 4, out_channels, kernel_size=(1, 1, 1), stride=(1, 1, 1), bias=False)
        self.out3 = nn.Conv3d(n_filters * 2, out_channels, kernel_size=(1, 1, 1), stride=(1, 1, 1), bias=False)
...
class ResVNet_DS(nn.Module):
    def __init__(self, in_channels=3, out_channels=2, n_filters=32, normalization='none', has_dropout=False):
        super(ResVNet_DS, self).__init__()
        self.has_dropout = has_dropout

        self.block_one = ResidualConvBlock(1, in_channels, n_filters, normalization=normalization)
        self.block_one_dw = DownsamplingConvBlock(n_filters, 2 * n_filters, normalization=normalization)
